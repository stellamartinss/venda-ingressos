...
function HomePage() {
  ...
  const adminLocations = useMemo(
    () =>
      JSON.parse(localStorage.getItem('adminLocations') || '[]') as string[],
    []
  );
  const adminCategories = useMemo(
    () =>
      JSON.parse(localStorage.getItem('adminCategories') || '[]') as string[],
    []
  );
  const visibleFilters = useMemo(
    () =>
      JSON.parse(
        localStorage.getItem('adminVisibleFilters') ||
  ...
  const hiddenLocations = useMemo(
    () =>
      JSON.parse(
        localStorage.getItem('adminHiddenLocations') || '[]'
      ) as string[],
    []
  );
  const hiddenCategories = useMemo(
    () =>
      JSON.parse(
        localStorage.getItem('adminHiddenCategories') || '[]'
      ) as string[],
    []
  );
        ...
        {filtered.map((ev) => (
          ...
          >
            ...
            <div className='p-4 space-y-2'>
              ...
              <Link
                to={`/evento/${ev.id}`}
                className='inline-block mt-2 px-4 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700'
              >
                Ver mais
              </Link>
            </div>
