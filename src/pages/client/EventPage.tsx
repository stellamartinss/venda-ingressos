...
function EventPage() {
  ...
  // const [order, setOrder] = useState<Quantities[]>([])
  ...
  const updateQuantity = (tId: string, value: number) => {
    setQuantities((prev) => {

      const existing = prev.find((q) => q.ticketTypeId === tId);

      if (existing) {
        return prev.map((q) =>
          q.ticketTypeId === tId ? { ...q, quantity: value } : q
        );
      }

      return [...prev, { ticketTypeId: tId, quantity: value }];
    });
  };

  function onBuy() {
    const userToken = localStorage.getItem('authUser');
    const organizerToken = localStorage.getItem('authToken');
            ...
            {tickets.map((t) => (
              <div key={t.id} className='flex items-center gap-2'>
                <button
                  ...
                  className={`flex-1 text-left p-3 rounded-md border hover:bg-gray-50 dark:hover:bg-gray-800 ${
                    selected?.id === t.id
                  ...
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <div className='text-xs opacity-70'>
                        Qtd. disponível: {t.quantityTotal}
                      </div>
                    </div>
                ...
                <input
                  ...
                  min={0}
                  value={
                    quantities.find((e) => e.ticketTypeId === t.id)?.quantity ||
                    0
                  }
                  onChange={(e) => {
                    setSelected(t)
